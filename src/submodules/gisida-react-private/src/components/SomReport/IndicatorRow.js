...
class IndicatorRow extends React.Component {
  static buildPieData(indicators) {
    const indicatorKeys = ['Recovered', 'Non-recovered', 'Defaulted', 'Died'];
    const populationKeys = ['boys', 'girls', 'pregnant'];
    const pieMap = {};
    let indicatorData;
    let datum = {};
  ...
  render() {
      ...
      if (services[indicatorKeys[i]]) {
        ...
        for (let p = 0; p < populationKeys.length; p += 1) {
          if (indicatorData[populationKeys[p]]) {
            ...
            population = populationData.count !== 0 && !Number.isNaN(populationData.count) ? (
              <tr>
                  ...
                  <ul className="indicator-bar">
                    <li className="indicators-section-header">
                      <a href="#">
                        ...
                        <PieChart
                          seriesName={''}
                          seriesData={indicatorData.Recovered
                            ? seriesData[indicatorKeys[i]].filter(x => x.y > 0 && x.y !== null)
        ...
        if (services.Admitted && services.Admitted[populationKeys[p]]) {
          ...
          stack = (
            <span
              ...
              style={{
                width: (services.totals === 'NaN%' || services.totals === 0)
                  ? '0%' : `${services.Admitted[populationKeys[p]].percent.toString()}%`,
    ...
    return services.totals !== null && services.totals !== '0' && services.totals !== 0 && typeof (services.totals) !== 'undefined' ?
    (
      <li>
        <li>
          <a className="service-name indicator-label" href="#" onClick={e => this.showIndicators(e)}>
            {services.type}
            <div className="totals">Total Admitted</div>
            <span className="indicator-totals indicator-value">{services.totals}</span>
            <span className={`total ${services.Recovered ? 'caret' : ''} caret-${services.isOpen ? 'down' : 'right'}`} />
