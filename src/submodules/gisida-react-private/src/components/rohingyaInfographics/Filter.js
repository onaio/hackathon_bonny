...
import buildFilterState from './filters/buildFilterState';
import clearFilterState from './filters/clearFilterState';
import { updateFilters, infosaveFilterState, closeFilter } from '../../store/actions';
import FilterSelector from './FilterSelector';
import './Filter.scss';

const mapStateToProps = (state, ownProps) => {
  const { DASHBOARD, VIEW, INFOFILTER } = state;
  const { sectorId } = ownProps;
  return {
    VIEW,
    DASHBOARD,
    INFOFILTER,
    sectorId,
    GLOBAL: state,
    showInfoFilterPanel: DASHBOARD.showFilter,
    showFilterBtn: true,
    sectorsObj: DASHBOARD.sectorsObj,
    sectorObj: DASHBOARD.sectorsObj[sectorId]
  }
}
...
export class Filter extends Component {
  ...
  getLayerFilter(sectorId) {
    let sectorObj;
    const { sectorsObj } = this.props;
    if (!sectorsObj || !sectorsObj.length) {
      return null;
    }
    for (let i = 0; i < sectorsObj.length; i += 1) {
      sectorObj = sectorsObj[i];
      if (sectorObj.id === sectorId) {
        return sectorObj.filters && sectorObj.filters.layerFilters;
      }
    }
    return null;
  }
  ...
  handleFilterClick() {
    const { dispatch, mapId, sectorId } = this.props;
    ...
    // dispatch(Actions.toggleFilter(mapId, sectorId));
  }

  componentWillReceiveProps(nextProps) {
    if (!nextProps.sectorObj) return false;

    const sectorId = nextProps.sectorObj.id;
    const { INFOFILTER } = nextProps;

    // Build new component state or retrieve it from FILTER store
    const filterState = nextProps.INFOFILTER[sectorId];
    const layerFilters = this.getLayerFilter(sectorId); // this may be deprecated
    const filterOptions = filterState && filterState.filterOptions
      ? filterState.filterOptions
      : nextProps.sectorObj.filterOptions || {};
    ...
    const doUpdate = (sectorId !== this.state.sectorId
      && (filterOptions && Object.keys(filterOptions).length > 0))
      || (filterState && filterState.doUpdate
        && (this.state.INFOFILTER && this.state.INFOFILTER[sectorId] && !this.state.INFOFILTER[sectorId].doUpdate));
    ...
    this.setState({
      ...
      if (doUpdate) nextProps.dispatch(updateFilters(sectorId));
    });
  }

  onCloseClick = (e) => {
    e.preventDefault();
    this.props.toggleFilterPanel(e);
    //TODO dispatch close action
  ...
  onClearClick = (e, isFilterable) => {
    ...
    const { sectorId, filterOptions } = this.state;
    const { dispatch, sectorObj, INFOFILTER } = this.props;
    // Clear layerFilter from mapbox layer
    // dispatch(Actions.setLayerFilter(mapId, sectorId, null));
    ...
    const isClear = true;
    clearFilterState(filterState, sectorId, dispatch, isClear);

    const { filters, isFiltered } = filterState;
    this.setState({
      filters,
      filterOptions,
      isFiltered,
  ...
  onApplyClick = (e, isFilterable) => {
    ...
    const { filters, sectorId, filterOptions } = this.state;

    const { sectorObj, dispatch } = this.props;
    if (!sectorObj) {
      return false;
    }
    ...
    if (!INFOFILTER[sectorId].originalsectorObj) {
      
      const newFilterState = buildFilterState(filterOptions, filters, sectorObj, isOr);
      const { originalsectorObj } = newFilterState;
      // dispatch(Actions.resetFilteredLayer(mapId, originalsectorObj));
    }

    const newFilterState = buildFilterState(filterOptions, filters, sectorObj, isOr);
    dispatch(infosaveFilterState(sectorId, newFilterState));

    if (regenStops) {
      const { fauxsectorObj } = newFilterState;
      // Reload layer to re-aggregate and re-add layer
      // this.props.dispatch(Actions.addLayer(mapId, fauxsectorObj));
    } else if (nextFilters.length > 1) {
      // Apply layerFilter to mapbox layer
      // this.props.dispatch(Actions.setLayerFilter(mapId, sectorId, nextFilters));
    }
  ...
  setFilterQueries = (filterKey, nextQueries, queriedOptionKeys) => {
    ...
    buildFilterState(filterOptions, nextFilters, sectorObj);
  }
  ...
  buildFilteredFilters = (clickedFilterKey, nextFilters) => {
    ...
    const newsectorObj = {
      aggregate,
      source: this.props.sectorObj.source,
      type: 'filteredFilter',
    };
    const newLayerOptions = generateFilterOptions(newsectorObj);
  ...
  isMapFiltered = () => {
    const layerFilters = this.getLayerFilter(this.state.sectorId);
    if (!layerFilters) {
      return false;
    }
  ...
  buildNextFilters = (nextOptions, filters, filterKey, isResetable) => {
    ...
    for (let i = 0; i < filterKeys.length; i += 1) {
      ...
      if (Object.keys(nextFilters[filterKey].options)
      .every(obj => nextFilters[filterKey].options[obj].enabled === true)) {
    ...
    if (isFiltered) {
      ...
      const layerFilters = this.getLayerFilter(this.props.sectorObj.id);
  ...
  render() {
    ...
    let isClearable;
    if (this.props && this.props.INFOFILTER && this.props.sectorObj) {
    const {INFOFILTER} = this.props;
    const { id } = this.props && this.props.sectorObj;
      if ((INFOFILTER && INFOFILTER[id] && INFOFILTER[id].aggregate && 
           INFOFILTER[id].aggregate['accepted-filter-values']
        && !INFOFILTER[id].aggregate['accepted-filter-values'].every(obj => obj === 'all'))
        && (INFOFILTER && INFOFILTER[id] && !INFOFILTER[id].isClear)) {
      isClearable = true;
      }
    }
    const doClear = isFilterable || this.isMapFiltered() || isClearable;
    let sidebarOffset = this.props.showInfoFilterPanel
      ? '260px'
    ...
    <div>
      <div style={{ float: 'right' }}>
      {
        ...
        <a href="#" className="infographics-filter"
        ...
        <div>
        {
          ...
          this.props.showInfoFilterPanel  ?
            <div>
              <div className={`profile-view-container info-filter-container${filterClasses}`}>
                {<button
                ...
                <div className="info-filter-footer">
...
Filter.propTypes = {
  sectorObj: PropTypes.objectOf(PropTypes.any),
  doShowProfile: PropTypes.bool.isRequired,
  showInfoFilterPanel: PropTypes.bool.isRequired,
  sectorsObj: PropTypes.arrayOf(PropTypes.any).isRequired,
